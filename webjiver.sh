...
cat amass | awk -F "-->" '{print $3}' | grep -E -v "RIROrganization|Netblock" | cut -d "(" -f 1 | grep -v "\\s+" | sed -E 's/^\s+//g' | grep -E -v "\S+:\S+:\S+:\S+::\S+" | grep -E -v "\S+:\S+:\S+::\S+:\S+"|grep -E -v "\S+:\S+:\S+:\S+:\S+:\S+:\S+:\S+" | grep -E -v "\S+:\S+:\S+::\S+" >dom.tmp
echo -e "\e[32mFinding open ports....\e[0m"
naabu -silent -list dom.tmp -o ports.tmp
...
katana -list validhttp.tmp -o katana.tmp

while read domain; do
	cat results/$domain.txt 2> /dev/null  1>> $output.tmp
done < dom.tmp

cat katana.tmp > $output.tmp
