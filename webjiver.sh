...
cat amass | awk -F "-->" '{print $3}' | grep -E -v "RIROrganization|Netblock" | cut -d "(" -f 1 | grep -v "\\s+" | sed -E 's/^\s+//g' | grep -E -v "\S+:\S+:\S+:\S+::\S+" >>dom.tmp
echo -e "\e[32mFinding open ports....\e[0m"
naabu -silent -list dom1.tmp -o ports.tmp
...
paramspider -l dom1.tmp > param.tmp
